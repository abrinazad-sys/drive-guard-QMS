...
import { Users, FolderOpen, AlertTriangle, Download, Cloud, RefreshCw, ChevronRight, Clock, Loader2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard label="Total Users" value={loadingUsers ? "..." : users.length} icon={<Users className="h-5 w-5" />} hint="Registered accounts" />
        <MetricCard label="Folders Managed" value={loadingFolders ? "..." : rootFolders.length} icon={<FolderOpen className="h-5 w-5" />} hint="Top-level folders" />
        <MetricCard label="Sync Failures" value={0} icon={<AlertTriangle className="h-5 w-5" />} hint="System healthy" variant="success" />
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Workspace</span><span className="font-medium">bedata.com</span></div>
              ...
              <Button variant="outline" size="sm" className="w-full mt-2" onClick={() => refetchFolders()}>
                <RefreshCw className="h-3 w-3 mr-2" />Refresh now
              </Button>
            </CardContent>
          </Card>
          <Card>
              ...
              {loadingFolders ? (
                ...
                <button key={f.id} onClick={() => nav("/documents")} className="flex items-center gap-3 w-full p-2 rounded-lg hover:bg-accent transition text-left">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground truncate">Department Folder</div>
                  </div>
                </button>
              ))}
            </CardContent>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <MetricCard label="Assigned Folders" value={loadingFolders ? "..." : rootFolders.length} icon={<FolderOpen className="h-5 w-5" />} />
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {loadingFolders ? (
            <div className="flex justify-center py-10 col-span-full"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
          ) : rootFolders.map(f => (
            <button key={f.id} onClick={() => nav("/documents")} className="text-left p-4 rounded-xl border border-border hover:border-primary hover:bg-accent transition group">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground truncate">Department Folder</div>
                  <div className="text-xs text-muted-foreground mt-2">{new Date(f.modifiedAt).toLocaleDateString()}</div>
                </div>
              </div>
            </button>
          ))}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm text-muted-foreground">Activity tracking will be available soon with real-time sync.</div>
        </CardContent>
      </Card>
    </div>
  );
